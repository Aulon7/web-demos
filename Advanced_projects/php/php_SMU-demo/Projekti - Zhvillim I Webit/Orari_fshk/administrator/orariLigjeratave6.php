...
//administratori i kycur
$usernameID = $_SESSION['usernameID'];

//selektimi i lendeve per paraqitjen e orarit per semestrin e gjashte
$querySql = "SELECT d.pershkrimi as dita, l.emri as lenda, CONCAT(pr.emri, ' ', pr.mbiemri) AS profesori, 
                    ...
                    WHERE s.id = 6 AND l.kodi = lgj.lenda AND lgj.semestri = s.id AND pr.id = lgj.profesori
                    AND d.id = lgj.dita and k.id = lgj.kohezgjatja";
$queryRes = mysqli_query($connect, $querySql);

//marrim te dhenat per vendosje ne tabela
